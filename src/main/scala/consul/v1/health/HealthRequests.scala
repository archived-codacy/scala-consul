...
import consul.v1.common.Types.{NodeId, ServiceType}
...
trait HealthRequests{
  ...
  def service(service: ServiceType,tag:Option[String]=Option.empty,passing:Boolean=false,dc:Option[String]=Option.empty): Future[Seq[NodesHealthService]]
...
object HealthRequests {
  ...
  def apply(basePath: String)(implicit executionContext: ExecutionContext): HealthRequests = new HealthRequests {
    ...
    def service(service: ServiceType, tag:Option[String], passing:Boolean=false,dc:Option[String]): Future[Seq[NodesHealthService]] = erased{
      lazy val params = (if(passing) List(("passing","")) else List.empty ) ++ tag.map{ case tag => ("tag"->tag) }
